...
const WishlistPage = (props: Props) => {
    ...
    const { mutate } = useMutation({
        mutationFn: async (productId: any) => {
            try {
                return await axios.delete(`http://localhost:8080/api/favorite/${productId}` ,{
                    headers: {
                        Authorization: `Bearer ${token}`, // Truyền token vào header
                    },
                })
                ...
                {favorite?.data.length <= 0 ?
                    ...
                    <div className="">
                        <div className="-mx-[15px] p-[24px_20px] lg:mx-0">
                            {favorite?.data.map((item: any, index: any) => (
                                <div className={`${index === 0 ? "" : "mt-[24px]"} flex justify-between items-center`}>
                                    <div className='w-[80%] flex'>
                                        <div className='w-[120px]'>
                                            <div
                                                className="pt-[123%] bg-cover bg-center bg-no-repeat"
                                                style={{
                                                    backgroundImage: `url(${item.productId.attributes[0].images[0]})`,
                                                }}
                                            ></div>
                                        </div>
                                        <div className="w-[calc(100%-120px)] pl-[16px] flex flex-col">
                                            ...
                                            <span className='mt-[14px] font-[700]'>{item.productId.attributes[0].variants[0].price} VND</span>
                                        </div>
                                    </div>
                                    <div onClick={() => mutate(item?.productId?._id)} className="hidden w-[20%] cursor-pointer text-center border border-[#E8E8E8] rounded-[3px] text-[14px] py-[6px] px-[8px] lg:block">Xóa khỏi mục yêu thích</div>
